...
export default function Show({ assessmentId }) {
    ...
    const handleDelete = async () => {
        const confirmed = await confirmDelete('Hapus Penilaian?', 'Tindakan ini tidak dapat dibatalkan.');
        if (!confirmed) return;
        setIsDeleting(true);
        try {
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                <h2 className="font-headline-lg-mobile md:font-headline-lg text-headline-lg-mobile md:text-headline-lg text-on-surface">
                    {assessment?.title || 'Detail Penilaian'}
                </h2>
                {assessment && (
                    <div className="flex flex-wrap items-center gap-2 mt-2">
                        <span className="bg-surface-container-highest text-on-surface-variant px-3 py-1 rounded-full font-label-sm text-label-sm">{assessment.chapter?.name || 'Tidak Dikategorikan'}</span>
                        <span className="text-on-surface-variant font-label-md text-label-md">• {assessment.duration} mnt</span>
                        <span className="text-on-surface-variant font-label-md text-label-md">• {assessment.questions?.length || 0} Soal</span>
                    </div>
                )}
            </div>
            {assessment && (
    ...
    if (loading) {
        return (
            <DashboardTemplate role="teacher" activeTab="assessments" headerSection={headerSection} showBack={true} onBack={handleBack}>
                <Head title="Detail Penilaian | LMS Diajar" />
    ...
    if (!assessment) {
        return (
            <DashboardTemplate role="teacher" activeTab="assessments" headerSection={headerSection} showBack={true} onBack={handleBack}>
                <Head title="Penilaian Tidak Ditemukan | LMS Diajar" />
                <div className="text-center py-12 text-on-surface-variant">Penilaian tidak ditemukan.</div>
            </DashboardTemplate>
        );
    }
    ...
    const chartData = [
        { label: '90-100 (A)', count: assessment.grade_distribution?.['90-100'] || 0, countLabel: `${assessment.grade_distribution?.['90-100'] || 0} siswa`, colorClass: 'bg-secondary', percentage: totalStudents > 0 ? ((assessment.grade_distribution?.['90-100'] || 0) / totalStudents) * 100 : 0 },
        { label: '80-89 (B)', count: assessment.grade_distribution?.['80-89'] || 0, countLabel: `${assessment.grade_distribution?.['80-89'] || 0} siswa`, colorClass: 'bg-primary', percentage: totalStudents > 0 ? ((assessment.grade_distribution?.['80-89'] || 0) / totalStudents) * 100 : 0 },
        { label: '70-79 (C)', count: assessment.grade_distribution?.['70-79'] || 0, countLabel: `${assessment.grade_distribution?.['70-79'] || 0} siswa`, colorClass: 'bg-primary-fixed-dim', percentage: totalStudents > 0 ? ((assessment.grade_distribution?.['70-79'] || 0) / totalStudents) * 100 : 0 },
        { label: '<70 (F)', count: (assessment.grade_distribution?.['0-49'] || 0) + (assessment.grade_distribution?.['50-59'] || 0) + (assessment.grade_distribution?.['60-69'] || 0), countLabel: `${(assessment.grade_distribution?.['0-49'] || 0) + (assessment.grade_distribution?.['50-59'] || 0) + (assessment.grade_distribution?.['60-69'] || 0)} siswa`, colorClass: 'bg-error', percentage: totalStudents > 0 ? (((assessment.grade_distribution?.['0-49'] || 0) + (assessment.grade_distribution?.['50-59'] || 0) + (assessment.grade_distribution?.['60-69'] || 0)) / totalStudents) * 100 : 0 },
    ];

    const studentAttempts = (assessment.attempts || []).map(att => ({
        studentId: att.student?.id,
        name: att.student?.full_name || att.student?.username || 'Tidak Diketahui',
        initials: (att.student?.full_name || att.student?.username || 'U').substring(0, 2).toUpperCase(),
        status: att.status === 'graded' ? 'Diserahkan' : att.status === 'submitted' ? 'Diserahkan' : 'Sedang Berjalan',
        timeText: att.completed_at ? `${moment(att.completed_at).diff(moment(att.started_at), 'minutes')}m` : 'Sedang Berjalan',
        score: att.grade,
                    ...
                    <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 shadow-sm">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center py-2 border-b border-outline-variant/30">
                                <span className="text-on-surface-variant font-label-sm">Durasi</span>
                                <span className="font-label-md text-on-surface">{assessment.duration} mnt</span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b border-outline-variant/30">
                                <span className="text-on-surface-variant font-label-sm">Upaya Maksimal</span>
                                <span className="font-label-md text-on-surface">{assessment.max_attempts}</span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b border-outline-variant/30">
                                <span className="text-on-surface-variant font-label-sm">Batas Lulus</span>
                                <span className="font-label-md text-on-surface">{assessment.pass_threshold}%</span>
                            </div>
                            {assessment.start_date && (
                                <div className="flex justify-between items-center py-2 border-b border-outline-variant/30">
                                    <span className="text-on-surface-variant font-label-sm">Mulai</span>
                                    <span className="font-label-md text-on-surface">{moment(assessment.start_date).format('MMM D, YYYY h:mm A')}</span>
                                </div>
                            )}
                            {assessment.due_date && (
                                <div className="flex justify-between items-center py-2">
                                    <span className="text-on-surface-variant font-label-sm">Tenggat</span>
