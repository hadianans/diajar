...
export default function Edit({ assessmentId }) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (err.response?.status === 422) {
                ...
                showError('Kesalahan', err.response?.data?.message || 'Kesalahan saat memperbarui penilaian');
            }
        } finally {
            setIsSaving(false);
        }
    ...
    const headerSection = (
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
                <h2 className="font-headline-lg-mobile md:font-headline-lg text-headline-lg-mobile md:text-headline-lg text-on-surface">Edit Penilaian</h2>
                <p className="font-body-md text-body-md text-on-surface-variant mt-1">Perbarui pengaturan dan penjadwalan penilaian.</p>
            </div>
    ...
    if (loading) {
        return (
            <DashboardTemplate role="teacher" activeTab="assessments" headerSection={headerSection}>
                <Head title="Edit Penilaian | LMS Diajar" />
    ...
    return (
        <DashboardTemplate role="teacher" activeTab="assessments" headerSection={headerSection} showBack={true} onBack={handleBack}>
            <Head title={`Edit ${formData.title || 'Penilaian'} | LMS Diajar`} />
            ...
            <div className="space-y-stack-lg max-w-screen-md mx-auto w-full pb-32">
                ...
                {Object.keys(errors).length > 0 && (
                    <div className="bg-error-container text-on-error-container p-4 rounded-xl border border-error/20">
                        <p className="font-label-md mb-2">Harap perbaiki kesalahan berikut:</p>
                ...
                <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-stack-lg shadow-sm space-y-stack-md">
                    ...
                    <div className="space-y-2">
                        <label className="font-label-md text-label-md text-on-surface">Judul <span className="text-error">*</span></label>
                        <input 
                    ...
                    <div className="space-y-2">
                        <label className="font-label-md text-label-md text-on-surface">Deskripsi</label>
                        <textarea 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-stack-md">
                        <div className="space-y-2">
                            <label className="font-label-md text-label-md text-on-surface">Durasi <span className="text-error">*</span></label>
                            <div className="relative">
                                ...
                                />
                                <span className="absolute right-4 top-1/2 -translate-y-1/2 font-label-sm text-outline">mnt</span>
                            </div>
                            {errors.duration && <p className="text-error text-label-sm">{errors.duration[0]}</p>}
                        </div>
                        <div className="space-y-2">
                            <label className="font-label-md text-label-md text-on-surface">Upaya Maksimal <span className="text-error">*</span></label>
                            <input 
                                className="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 text-body-md focus:ring-2 focus:ring-primary" 
                                type="number" 
                                min="1"
                ...
                <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-stack-lg shadow-sm space-y-stack-md">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-stack-md">
                        <div className="space-y-2">
                            <label className="font-label-md text-label-md text-on-surface">Tanggal Mulai</label>
                            <input 
                                className="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 text-body-md focus:ring-2 focus:ring-primary" 
                                type="datetime-local"
                        ...
                        <div className="space-y-2">
                            <label className="font-label-md text-label-md text-on-surface">Tanggal Tenggat</label>
                            <input 
                                className="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 text-body-md focus:ring-2 focus:ring-primary" 
                                type="datetime-local"
                        ...
                        <div className="space-y-2">
                            <label className="font-label-md text-label-md text-on-surface">Batas Lulus <span className="text-error">*</span></label>
                            <div className="relative">
                                <input 
                                    className="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 text-body-md focus:ring-2 focus:ring-primary" 
                                    type="number" 
                ...
                {assessment.questions && assessment.questions.length > 0 && (
                    <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-stack-lg shadow-sm space-y-stack-md">
                        <h3 className="font-headline-md text-headline-md text-on-surface flex items-center gap-2">
                            ...
                            Soal ({assessment.questions.length})
                        </h3>
                        <p className="text-on-surface-variant font-body-md">Soal tidak dapat diubah setelah pembuatan. Untuk mengubah soal, buat penilaian baru.</p>
                        <div className="space-y-2">
                            {assessment.questions.map((aq, idx) => {
                                ...
                                return (
                                    <div key={aq.id || idx} className="flex gap-4 p-3 rounded-lg bg-surface-container-low">
                                        ...
                                        <p className="text-body-md text-on-surface truncate flex-1">{plainText || 'Soal'}</p>
                                        <span className="font-label-sm text-primary shrink-0">{q.score || 0} poin</span>
                                    </div>
                                );
                            })}
                        </div>
